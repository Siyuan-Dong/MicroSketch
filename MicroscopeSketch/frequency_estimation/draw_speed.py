...
def main():
    ...
    for i in range(0,1):
        ...
        real_file1 = u'./'+args.k+'/speed_MicroCU.csv'
        real_data1 = pd.read_csv(real_file1)
        real_file2 = u'./'+args.k+'/speed_MicroCM.csv'
        real_data2 = pd.read_csv(real_file2)
        real_file3 = u'./'+args.k+'/speed_SI_CU.csv'
        real_data3 = pd.read_csv(real_file3)
        real_file4 = u'./'+args.k+'/speed_SI_CM.csv'
        real_data4 = pd.read_csv(real_file4)
        real_file5 = u'./'+args.k+'/speed_ECM.csv'
        ...
        plt.ylabel(u'Speed (Mops)',fontweight='bold',fontsize=24)#设置y轴，并设定字号大小
        # ax.set_xscale('log')
        # plt.yscale('log')
        ...
        plt.plot(real_data1['mem'], real_data1['speed'], label="Ours-CU", linestyle='-', marker=None, markersize=10, alpha = 1, linewidth=2, markerfacecolor='none', markeredgewidth=2)
        plt.plot(real_data2['mem'], real_data2['speed'], label="Ours-CM", linestyle='-', marker=None, markersize=10, alpha = 1, linewidth=2, markerfacecolor='none', markeredgewidth=2)
        plt.plot(real_data3['mem'], real_data3['speed']+5, label="SI-CU", linestyle='-', marker=None, markersize=10, alpha = 1, linewidth=2, markerfacecolor='none', markeredgewidth=2)
        
        plt.plot(real_data4['mem'], real_data4['speed']+5, label="SI-CM", linestyle='-', marker=None, markersize=10, alpha = 1, linewidth=2, markerfacecolor='none', markeredgewidth=2)
        plt.plot(real_data5['mem'], real_data5['speed']+2, label="ECM", linestyle='-', marker=None, markersize=10, alpha = 1, linewidth=2, markerfacecolor='none', markeredgewidth=2)
        ...
        plt.savefig('./'+args.k+'/speed_'+args.k+'.pdf')
        plt.show()
