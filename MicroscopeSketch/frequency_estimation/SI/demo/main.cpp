...
void Read_File(int argc,char*argv[]){
    ...
    while(fread(key, TRACE_L, 1, file) > 0)
    {
        ...
        cout << CM_AEE / totcnt << "," << CU_AEE / totcnt << endl;
        string tm = string(argv[2]) + "/SI_CM.csv";
        ofstream fout(tm.c_str(),ios::app);
        fout<<int(mymemory*1000)<<","<<CM_AEE/totcnt<<","<<endl;
        fout.close();
        tm = string(argv[2]) + "/SI_CU.csv";
        fout.open(tm.c_str(),ios::app);
        fout<<int(mymemory*1000)<<","<<CU_AEE/totcnt<<","<<endl;
        fout.close();
#ifndef tpTest
#else
