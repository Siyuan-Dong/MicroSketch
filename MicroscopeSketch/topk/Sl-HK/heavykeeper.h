...
{
    public:
        struct node {int* C ;uint32_t FP;} HK[HK_d][MAX_MEM+10];
        BOBHash64 * bobhash;
        int M2;
        int cycle;
        double time_now;
        ...
        //hash function
        unsigned long long Hash(int j, uint8_t *ST, int len)
        {   
            bobhash = new BOBHash64(j + 10);
            unsigned long long ans = (bobhash->run((char *)ST, len));
            delete bobhash;
        ...
        //update an item
        void Insert(uint8_t *x, int len, int time_stamp)
        {
            //用一层
        ...
        //query an item
        int num_query(uint8_t *x, int len){
